...
# The block below is newly added
import subprocess
import sys
import tarfile
...
from diffusers.loaders import LoraLoaderMixin, text_encoder_lora_state_dict  # noqa
...
def arg_as_bool(value: Any) -> bool:
    ...
    if value.lower() in ("no", "false", "f", "n", "0"):
        return False
    raise argparse.ArgumentTypeError("A boolean value is expected.")
...
def delete_expt_model_tar_gz(
    ...
    if excl_filenames is None:
        ...
        excl_filenames = [excl_filenames]

    for item in os.listdir(tgt_dir):
        item_path = os.path.join(tgt_dir, item)

        if item not in excl_filenames:
            if os.path.isfile(item_path):
                os.remove(item_path)
                msg = f"The file '{item_path}' has been removed."
                log_or_print(msg, logger)
            elif os.path.isdir(item_path):
                shutil.rmtree(item_path)
                msg = f"The directory '{item_path}' has been removed."
                log_or_print(msg, logger)
...
def parse_args(input_args=None):
    ...
    # The argument below is newly added
    parser.add_argument(
        "--compress_output",
        default=False,
        type=arg_as_bool,
    ...
    if args.with_prior_preservation:
        args.class_data_dir = f"{args.instance_data_dir}/class"  # newly added
...
def main(args):
            ...
            with accelerator.accumulate(unet):
                ...
                if not args.train_text_encoder:
                    ...
                    unet_added_conditions.update(
                        {"text_embeds": pooled_prompt_embeds.repeat(elems_to_repeat, 1)}
                    )
                    prompt_embeds_input = prompt_embeds.repeat(elems_to_repeat, 1, 1)
                    model_pred = unet(
                        noisy_model_input,
    ...
    if args.compress_output:  # newly added
        compress_dir_to_model_tar_gz(args.output_dir)  # newly added
        delete_expt_model_tar_gz(args.output_dir, "model.tar.gz")  # newly added
